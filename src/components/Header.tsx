  ...
  return (
    <header className="sticky top-0 z-50 bg-white shadow-sm px-6 lg:px-12 py-4 flex justify-between items-center">
      ...
      <nav className="hidden md:flex gap-8 items-center">
        {['Cities', 'Gifts', 'Our Story'].map((item) => (
          <Link 
            key={item} 
        ...
        <Link href="/cart" className="text-sm font-medium uppercase tracking-widest text-slate-black hover:text-champagne-gold transition-colors">
          Cart (0)
        </Link>
      </nav>
